...
const StudentSemester = () => {
  ...
  const upgradeStudents = async () => {
    ...
    try {
      const response = await axios.post(
        "https://e-college-data.onrender.com/v1/students/upgradeStudent",
        {
          course_code: courseCode,
          sem: semester,
        }
      ...
      setUpgradeSuccess(
        `Students successfully upgraded from semester ${semester} to ${nextSemester}`
      );
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto bg-[#101828]">
      ...
      <div className="flex flex-wrap gap-4 mb-6 text-violet-300">
        <div className="flex flex-col">
          <label className="mb-2 text-sm font-bold ">Course</label>
          <select
            value={courseCode}
            onChange={(e) => setCourseCode(e.target.value)}
            className="p-2 border border-gray-300 rounded-md bg-slate-800"
          >
      ...
      <div className="mb-6">
        ...
        >
          {loading
            ? "Processing..."
            : `Upgrade Students to Semester ${Math.min(
                parseInt(semester) + 1,
                8
              )}`}
        </button>
      ...
      {showConfirmation && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
            ...
            <p className="mb-6">
              Are you sure you want to upgrade all students from Semester{" "}
              {semester} to Semester {Math.min(parseInt(semester) + 1, 8)}? This
