...
import { UserCheck, Users, Award, BookOpen, Save, X, Plus, Trash2, Search } from 'lucide-react';
...
const TeacherCourseManagement = () => {
  ...
  const fetchTeachers = async () => {
    ...
    try {
      const response = await fetch('https://e-college-data.onrender.com/v1/teachers/teachers', {
        method: 'POST',
        headers: {
  ...
  const assignHOD = async (c_roll, course_code) => {
    ...
    try {
      const response = await fetch('https://e-college-data.onrender.com/v1/teachers/teachers-hod', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      setSuccess(`Successfully assigned teacher as HOD for course ${course_code}`);
  ...
  const updateTeacherCourseCodes = async (c_roll, courseCodes) => {
    ...
    try {
      const response = await fetch('https://e-college-data.onrender.com/v1/teachers/teacher-coursecode-update', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const removeCourseCode = (courseCodeToRemove) => {
    setEditingCourseCodes(editingCourseCodes.filter(code => code !== courseCodeToRemove));
  };
  ...
  const availableCourseCodes = ['101', '102', '103', '104', '105', '301', '302'];
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Manage teacher course assignments and HOD positions</p>
        </div>
        ...
        {loading && teachers.length === 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">No teachers found matching "{searchTerm}"</p>
            <button
              onClick={() => setSearchTerm('')}
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              ...
              <div key={teacher._id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                {/* Teacher Header */}
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl font-semibold text-gray-800 flex items-center gap-2">
                      {teacher.name}
                      {teacher.hod && (
                        <Award className="w-5 h-5 text-yellow-500" title="HOD" />
                      )}
                    </h3>
                    <p className="text-gray-600">{teacher.c_roll}</p>
                ...
                <div className="space-y-3 mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-600">₹{teacher.salary?.toLocaleString() || 'Not set'}</span>
                  </div>
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-700">Course Codes:</span>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {editingCourseCodes.map((code) => (
                          <span key={code} className="bg-indigo-100 text-indigo-800 text-xs px-2 py-1 rounded-full flex items-center gap-1">
                            {code}
                            <button
                      ...
                      <div>
                        <div className="text-xs text-gray-600 mb-1">Add course codes:</div>
                        <div className="flex flex-wrap gap-1">
                          {availableCourseCodes
                            .filter(code => !editingCourseCodes.includes(code))
                            .map((code) => (
                              <button
                                key={code}
                    ...
                    <div className="flex flex-wrap gap-1">
                      {teacher.course_code?.length > 0 ? (
                        teacher.course_code.map((code) => (
                          <span key={code} className="bg-indigo-100 text-indigo-800 text-xs px-2 py-1 rounded-full">
                            {code}
                          </span>
                        ))
                      ) : (
                ...
                <div className="pt-4 border-t border-gray-200">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-700">HOD Status:</span>
                    <div className="flex items-center gap-2">
                      {teacher.hod ? (
                        ...
                        <div className="flex items-center gap-2">
                          <select
                            ...
                            disabled={loading || getHODEligibleCourses(teacher).length === 0}
                          >
                            <option value="">
        ...
        {teachers.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">No teachers found. Click "Refresh Teachers" to load data.</p>
          </div>
        )}
      </div>
    </div>
