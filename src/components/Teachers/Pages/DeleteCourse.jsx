...
export default function ScheduleViewer() {
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        ...
        const response = await axios.post(
          `https://e-college-data.onrender.com/v1/adminroutine/routine-all/${courseCode}/${selectedSemester}`
        );
        ...
        if (response.data?.data?.[0]) {
          ...
          setError("No schedule data available for this semester");
        }
      } catch (err) {
    ...
    fetchSchedule();
  }, [refreshTrigger, selectedSemester]);
  ...
  // Create array of semester options
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h1 className="text-2xl font-bold">
          Class Schedule
          {scheduleData && ` - ${scheduleData.course_name}`}
        </h1>

        <div className="flex items-center gap-4">
          {/* Semester Dropdown */}
          <div className="flex items-center">
            <label htmlFor="semester" className="mr-2 text-gray-700">
              Semester:
            </label>
            <select
              id="semester"
              ...
              className="border rounded-md px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
      ...
      {!loading && !error && !scheduleData && (
        <div className="p-4 text-center border rounded-lg shadow-sm">
          <p className="text-gray-600 mb-4">
            No schedule data available for Semester {selectedSemester}
          </p>
          <button
            onClick={() => setRefreshTrigger((prev) => prev + 1)}
                ...
                <div className="divide-y">
                  {classes.map((cls) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          handleDelete(day, cls._id, cls.paper_code)
                        }
